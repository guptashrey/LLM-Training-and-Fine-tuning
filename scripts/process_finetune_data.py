...
logger = logging.getLogger("Process fine-tune data")
logger.setLevel(logging.INFO)

def preprocess(data_directory):
    '''
    Process the webpages text data for fine-tuning LLMs.
    ...
    for i in tqdm(preprocessed_docs[183:200]):
        ...
        q_a_list.extend(json.loads(response.choices[0].message.content))

    with open('../data/fine_tune_data.json', 'w') as outfile:
        json.dump(q_a_list, outfile)
